...
def evaluate_calculation():
    ...
    try:
        ...
        calculation = str(eval(calculation, {"__builtins__": None},
                               {  # dictionar plus conversie la string si paseaza la "calculation"
        ...
        text_result.insert(1.0,
                           "Error")  # genereaza un msj de eroare daca nu este bun calculul(err de sintaxa/impartire cu zero)
...
root.geometry("340x330")  # dimensiunile
...
for (text, row, col) in buttons:
    tk.Button(root, text=text, command=lambda t=text: add_to_calculation(t), width=5, font=("Arial", 14), bg="#d1e7dd",
              fg="black").grid(row=row, column=col)  # creeaza buton pt fiecare chestie din lista
...
for (text, row, col) in operations:
    tk.Button(root, text=text, command=lambda t=text: add_to_calculation(t), width=5, font=("Arial", 14), bg="#f8d7da",
              fg="black").grid(row=row, column=col)
...
for i, (text, symbol) in enumerate(functions):  # enumerate ca sa nu mai scriem manual toate randurile si coloanele
    tk.Button(root, text=text, command=lambda s=symbol: add_to_calculation(s), width=5, font=("Arial", 14),
              bg="#cff4fc", fg="black").grid(row=i + 2, column=5)

# butoane aditionale
tk.Button(root, text='a', command=set_a_value, width=5, font=("Arial", 14), bg="#e2e3e5", fg="black").grid(row=6,
                                                                                                           column=1,
                                                                                                           columnspan=1)
tk.Button(root, text='b', command=set_b_value, width=5, font=("Arial", 14), bg="#e2e3e5", fg="black").grid(row=6,
                                                                                                           column=2,
                                                                                                           columnspan=1)
tk.Button(root, text='c', command=set_c_value, width=5, font=("Arial", 14), bg="#e2e3e5", fg="black").grid(row=6,
                                                                                                           column=3,
                                                                                                           columnspan=1)
tk.Button(root, text='Plot', command=plot_graph, width=5, font=("Arial", 14), bg="#d1e7dd", fg="black").grid(row=6,
                                                                                                             column=4,
                                                                                                             columnspan=1)

# butoane pentru stergere si egal
tk.Button(root, text='C', command=clear_field, width=11, font=("Arial", 14), bg="#f8d7da", fg="black").grid(row=7,
                                                                                                            column=3,
                                                                                                            columnspan=2)
tk.Button(root, text='=', command=evaluate_calculation, width=11, font=("Arial", 14), bg="#d1e7dd", fg="black").grid(
